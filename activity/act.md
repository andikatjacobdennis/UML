...
- If invalid or timed out: handle errors or cancel the order, with a retry loop if attempts remain
...
- **Flow Final Node**: A circle with an 'X' terminates a specific path (e.g., order cancellation due to timeout or manual cancellation)

**Associations**:

- The **Initial Node** links to the first action ("Display Order Summary") via a control flow
- The **Activity Final Node** is reached from the last action ("End Checkout Process")
...
Example: "Enter Payment Details" flows to "Validate Payment and Inventory" with [Payment Info] as an object (implicit in this model).
...
- **Merge Node**: Implicitly handled by loop-back or termination (no explicit merge in this version)

Example: After validation, a decision on [Payment Successful?] branches to success or error paths, with a retry loop or cancellation.
...
A time event after "Process Payment via Gateway" triggers "Auto-Cancel Order" if inactive, adding realism for e-commerce timeouts.
...
- **System**: "Display Order Summary," "Validate Payment and Inventory," "Check Timeout," "Process Payment via Gateway," "Handle Payment Error," "Generate Order Number," "End Checkout Process"
- **Payment Gateway**: Implicitly handles "Process Payment via Gateway" within System lane (as per reviewer's structure)
...
| **Description** | Manages the checkout process with payment validation, parallel updates, and timeout handling, including retry logic. |
| **Preconditions** | Cart contains items; checkout initiated. |
| **Postconditions** | Order completed or canceled with appropriate status. |
| **Main Flow** | 1. Initial Node → Display Order Summary (System). <br> 2. Enter Payment Details (Customer). <br> 3. Validate Payment and Inventory (System). <br> 4. Fork: Check Timeout (System) || Process Payment via Gateway (System). <br> 5. Decision: [Timed out?] → Auto-Cancel Order (Flow Final); [not timed out] → continue. <br> 6. Decision: [Payment Successful?] → Fork (Update Inventory || Send Confirmation Email) → Join → Generate Order Number; [not successful] → Handle Payment Error. <br> 7. Decision: [Allow Retry? and Attempts remaining?] → Clear Previous Payment Data → Loop to Enter Payment Details; [no] → Cancel Checkout (Flow Final). <br> 8. Activity Final Node via End Checkout Process. |
...
| **Exceptions** | **E1: Timeout** - Triggers Auto-Cancel Order. <br> **E2: Gateway Failure** - Leads to Handle Payment Error. |
| **Notes** | Uses swimlanes for clarity; time events reflect real-world constraints. Retry loop enhances user experience. |
...
Below is the updated PlantUML script for the "Checkout" Activity Diagram incorporating the reviewer's corrections.
